---
title: Learn to Write in English
comments: true
mathjax: true
abbrlink: 235ae210
categories:
  - [Languages, Writing]
tags:
  - English Writing
date: 2019-05-03 15:26:15
updated:
---


# Words and phrases

comb through, 梳理，/kom/
> While recovery teams continue <mark>combing through</mark> the test site at Cape Canaveral where SpaceX's first space-worthy Crew Dragon capsule was destroyed in an explosive accident last month, ... 



# Math-related

Substitute (10.5) into (10.3) and then <mark>dissect out the dependence on</mark> one of the fastors $q_j(\bm{Z}_j)$.

Denoting $q_j(\bm{Z}_j)$ by simply $q_j$ to <mark>keep the notation uncluttered</mark>,...